...
from IoU_and_Errors import intersection_over_union_and_errors

def mean_average_precision_and_errors(
    ...
    # list storing all duration errors (Relative percentage duration errors)
    duration_errors = []
    
    # list storing all time mark errors (start and end time marks)
    start_time_errors = []
    end_time_errors = []

    # list storing all duration biases (Relative percentage duration biases)
    duration_biases = []
    
    # list storing all time mark biases (start and end time marks)
    start_time_biases = []
    end_time_biases = []
    ...
    for c in range(num_classes):
        ...
        for detection_idx, detection in enumerate(detections):
            ...
            if num_gts > 0:
                iou, dur_err, st_err, et_err,\
                dist, dur_bias, st_bias, et_bias = intersection_over_union_and_errors(
            ...
            if best_dist < iou_threshold:
                ...
                if amount_bsegments[int(detection[0])][best_gt_idx] == 0:
                    ...
                    duration_errors.append(dur_err[0][best_gt_idx])
                    start_time_errors.append(st_err[0][best_gt_idx])
                    end_time_errors.append(et_err[0][best_gt_idx])
                    
                    duration_biases.append(dur_bias[0][best_gt_idx])
                    start_time_biases.append(st_bias[0][best_gt_idx])
                    end_time_biases.append(et_bias[0][best_gt_idx])
                else:
                    FP[detection_idx] = 1
    ...
    if sum(TP) > 0:
        return sum(average_precisions) / len(average_precisions),\
               sum(duration_errors) / len(duration_errors),\
               sum(start_time_errors) / len(start_time_errors),\
               sum(end_time_errors) / len(end_time_errors), \
               sum(duration_biases) / len(duration_biases),\
               sum(start_time_biases) / len(start_time_biases),\
               sum(end_time_biases) / len(end_time_biases), \
               100 * sum(TP) / total_true_bsegments
    else:
        return sum(average_precisions) / len(average_precisions),\
